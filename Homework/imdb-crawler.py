...
def main():
    ...
    for i, url in enumerate(url_strings):  # Enumerate, a great Python trick!
        ...
        # DELETE FOR FINAL!!! MAKES FOR LOOP RUN 10 TIMES!!!!!
        if i > 10:
            break
        ######################################################
...
def scrape_movie_page(dom):
    ...
    for movie in dom.by_tag('td#overview-top'):
        duration = re.search(r'\d+', movie.by_tag('time')[0].content).group()
        rating = movie.by_attr(itemprop="ratingValue")[0].content
        ratingcount = movie.by_attr(itemprop="ratingCount")[0].content
        for header in movie.by_tag('h1.header'):
            title = header.by_tag('span.itemprop')[0].content
        for year in header.by_tag('span.nobr'):
            years = year.by_tag('a')[0].content
        for genres in movie.by_tag('div.infobar'):
            for a in genres.by_tag('a'):
                try:
                    genre.append(a.by_tag('span')[0].content)
                except:
                    pass
                genreStr = ', '.join(genre)
        for directors in movie.by_attr(itemprop="director"):
            director = directors.by_tag('span')[0].content
        for writers in movie.by_attr(itemprop="creator"):
            for a in writers.by_tag('a'):
                try:
                    writer.append(a.by_tag('span')[0].content)
                except:
                    pass
                writerStr = ', '.join(writer)
        for actors in movie.by_attr(itemprop="actors"):
            for a in actors.by_tag('a'):
                try:
                    actor.append(a.by_tag('span')[0].content)
                except:
                    pass
                actorStr = ', '.join(actor)
    ...
    return [title, years, duration, genreStr, director, writerStr, actorStr, rating, ratingcount]
    #, duration, genres, directors, writers, actors, rating, \
    #    n_ratings
